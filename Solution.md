...
- Replaced the sample application with the Vietnamese drug-law RAG chatbot
  from the earlier group project.
- `POST /chat` preserves the group API contract: message, history, and `top_k`.
- `POST /ask` adds server-side conversation history using Redis.
- Retrieval uses the group's checked-in legal/news corpus with BM25, avoiding
  large model downloads on the free deployment instance.
- OpenAI generation is optional; a cited extractive answer is returned when no
...
- Health check: `https://day12-agent-pjde.onrender.com/health`
- API documentation: `https://day12-agent-pjde.onrender.com/docs`
- Source agent: `https://github.com/Morpho7137/2A202600677_Nguyen-Anh-Kiet/tree/main/group_project`
